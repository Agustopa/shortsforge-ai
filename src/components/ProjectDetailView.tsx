...
export const ProjectDetailView: React.FC<ProjectDetailViewProps> = ({
  ...
  const [videoValidationData, setVideoValidationData] = useState<any | null>(null);
  const [isValidatingVideo, setIsValidatingVideo] = useState(false);

  const fetchVideoDiagnostics = useCallback(async () => {
    if (!project.videoUrl) return;
    setIsValidatingVideo(true);
    try {
      const res = await safeFetchJson<any>(`/api/v1/projects/${project.id}/validate-video`);
      if (res?.success && res.validation) {
        setVideoValidationData(res.validation);
      }
    } catch (e) {
  ...
  useEffect(() => {
    if (project.videoUrl && activeTab === 'qc') {
      fetchVideoDiagnostics();
    }
  }, [project.videoUrl, activeTab, fetchVideoDiagnostics]);

  const handleDownloadVideo = async () => {
    if (!project.videoUrl) return;
    setIsDownloadingVideo(true);
    setDownloadProgressMsg('Preparing download...');

    const downloadEndpoint = `/api/v1/projects/${project.id}/download`;
    const cleanTitle = (project.title || 'ShortsForge_Video')
      .replace(/[^a-zA-Z0-9_\-\s]/g, '')
      .trim()
      .replace(/\s+/g, '_')
      .slice(0, 45);
    const filename = `ShortsForge_${cleanTitle}_${project.id}.mp4`;

    await downloadVideoFile(downloadEndpoint, {
      filename,
      onProgress: (_pct, msg) => setDownloadProgressMsg(msg),
      onSuccess: () => {
        setIsDownloadingVideo(false);
        setDownloadProgressMsg('Downloaded successfully!');
        setTimeout(() => setDownloadProgressMsg(null), 3000);
      },
      onError: (err) => {
        setIsDownloadingVideo(false);
        setDownloadProgressMsg(`Download error: ${err}`);
        setTimeout(() => setDownloadProgressMsg(null), 5000);
      }
    });
  };
    ...
    <div className="p-3.5 sm:p-6 md:p-8 max-w-7xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4 pb-3 sm:pb-4 border-b border-slate-800">
        <div className="space-y-1">
          <button
            onClick={onBack}
            className="text-xs font-semibold text-slate-400 hover:text-amber-400 flex items-center gap-1 transition-colors cursor-pointer min-h-[36px]"
          >
            <span>← Back to Dashboard</span>
          </button>
          <h2 className="text-lg sm:text-xl md:text-2xl font-black text-white tracking-tight line-clamp-1">
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1 sm:pt-0">
          {project.videoUrl && (
            <div className="relative inline-flex flex-col items-end">
              <button
                ...
                className="min-h-[44px] px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-slate-950 font-bold text-xs shadow-lg shadow-emerald-950/40 flex items-center gap-2 cursor-pointer disabled:opacity-50 transition-all"
                title="Download H.264 MP4 (Compatible with Windows, Mac, iPhone, Android, VLC, Social Media)"
              >
                <Download className={`w-4 h-4 ${isDownloadingVideo ? 'animate-bounce' : ''}`} />
                <span>{isDownloadingVideo ? 'Downloading MP4...' : 'Download MP4'}</span>
              </button>
              {downloadProgressMsg && (
                <span className="absolute -bottom-5 right-0 text-[10px] font-mono text-emerald-400 whitespace-nowrap bg-slate-950/90 px-2 py-0.5 rounded border border-slate-800">
                  {downloadProgressMsg}
                </span>
              )}
            </div>
          ...
          <button
            ...
            className="min-h-[44px] px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs shadow-lg shadow-amber-950/40 flex items-center gap-2 cursor-pointer disabled:opacity-50"
          >
          ...
          <button
            ...
            className="min-h-[44px] px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-amber-400 font-semibold text-xs border border-slate-700 flex items-center gap-1.5 cursor-pointer"
          ...
          <button
            onClick={onDuplicate}
            className="min-h-[44px] min-w-[44px] p-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs border border-slate-700 flex items-center justify-center cursor-pointer"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          {activeTab === 'qc' && (
            <div className="space-y-4">
              {/* MP4 Compatibility & Codec Verification Panel */}
              <div className="p-4 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900 to-emerald-950/20 border border-emerald-500/30 space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-slate-800">
                  <div>
                    <h3 className="font-bold text-sm text-slate-100 flex items-center gap-2">
                      <ShieldCheck className="w-4 h-4 text-emerald-400" />
                      <span>MP4 Codec & Playback Compatibility Inspector</span>
                    </h3>
                    <p className="text-xs text-slate-400 mt-0.5">
                      Strict FFprobe verification ensures high compatibility across Windows Media Player, QuickTime, iOS, Android, VLC, TikTok, Reels, and YouTube Shorts.
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={fetchVideoDiagnostics}
                      disabled={isValidatingVideo || !project.videoUrl}
                      className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-[11px] font-semibold flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                    >
                      <RefreshCw className={`w-3 h-3 ${isValidatingVideo ? 'animate-spin' : ''}`} />
                      <span>{isValidatingVideo ? 'Probing...' : 'Re-probe Stream'}</span>
                    </button>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1">
                  <div className="p-2.5 rounded-xl bg-slate-950/70 border border-slate-800 text-center">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500 block font-semibold">Video Codec</span>
                    <span className="font-mono text-xs font-bold text-emerald-400">
                      {videoValidationData?.videoCodec?.toUpperCase() || 'H.264 / AVC'}
                    </span>
                    <span className="text-[9px] text-slate-500 block">High Profile @ L4.0</span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-slate-950/70 border border-slate-800 text-center">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500 block font-semibold">Audio Codec</span>
                    <span className="font-mono text-xs font-bold text-emerald-400">
                      {videoValidationData?.audioCodec?.toUpperCase() || 'AAC (LC)'}
                    </span>
                    <span className="text-[9px] text-slate-500 block">48 kHz Stereo / 192k</span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-slate-950/70 border border-slate-800 text-center">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500 block font-semibold">Pixel Format</span>
                    <span className="font-mono text-xs font-bold text-emerald-400">
                      {videoValidationData?.pixFmt || 'YUV420P'}
                    </span>
                    <span className="text-[9px] text-slate-500 block">8-bit Standard BT.709</span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-slate-950/70 border border-slate-800 text-center">
                    <span className="text-[10px] uppercase tracking-wider text-slate-500 block font-semibold">FastStart (moov atom)</span>
                    <span className="font-mono text-xs font-bold text-emerald-400">
                      ENABLED (+faststart)
                    </span>
                    <span className="text-[9px] text-slate-500 block">Instant Web / Player Seek</span>
                  </div>
                </div>

                {videoValidationData && (
                  <div className="p-2.5 rounded-xl bg-slate-950/90 border border-slate-800 text-[11px] font-mono text-slate-400 flex flex-wrap items-center justify-between gap-2">
                    <div>
                      <span className="text-slate-300 font-bold">FFprobe Container:</span> {videoValidationData.formatName || 'mp4,mov,m4a'}
                      {videoValidationData.duration && <span className="ml-3"><span className="text-slate-300 font-bold">Duration:</span> {videoValidationData.duration.toFixed(2)}s</span>}
                      {videoValidationData.width && <span className="ml-3"><span className="text-slate-300 font-bold">Res:</span> {videoValidationData.width}x{videoValidationData.height}</span>}
                    </div>
                    {project.videoUrl && (
                      <button
                        onClick={handleDownloadVideo}
                        className="px-3 py-1 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/40 text-xs font-bold flex items-center gap-1.5 cursor-pointer"
                      >
                        <Download className="w-3.5 h-3.5" />
                        <span>Download Tested MP4</span>
                      </button>
                    )}
                  </div>
                )}
              ...
              <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-sm text-slate-100 flex items-center gap-2">
                    ...
                    <span>Automated Pipeline Quality Verification</span>
                  </h3>
